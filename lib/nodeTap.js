...
module.exports = function (options, mainCb) {
	...
	function runTests(cb) {
		grunt.util.async.forEachSeries(files, function (file, eCb) {
			...
			proc.stderr.on('data', function (data) {
				...
				result.unhandledExceptions.push(data);
				tapConsumer.write(data);
			...
			tapConsumer.on('end', function () {
				...
				if(result.unhandledExceptions.length) {
					result.testsPassed = false;
					tapConsumer.results.ok = false;
					result.fileOutput += result.unhandledExceptions.join('\n');
				}

				var stats = statsToString(shortFile, tapConsumer.results, result);
				...
				if (outputType === 'failures') {
					var failedTests = _(tapConsumer.results).reject('ok').valueOf();
	...
	function statsToString(file, tapResult, result) {
		result.unhandledExceptions = result.unhandledExceptions || [];
		var s = util.format("Stats: %s: %d/%d", file, tapResult.passTotal, tapResult.testsTotal);

		if(result.unhandledExceptions.length) {
			var lf = grunt.util.linefeed;
			s += lf + 'Uncaught exceptions: ' + lf + result.unhandledExceptions.join(lf);
		}
		return s;
	}
